...
import type { BookingFormData, Hotel } from '@/types/booking';
...
export function BookingSuccessPage() {
  ...
  useEffect(() => {
    if (!state?.reference) navigate('/', { replace: true });
  }, [state, navigate]);

  if (!state?.reference) return null;

  const { reference, formData, hotel, totalPrice, nights } = state;
                ...
                <div className="bg-slate-50 rounded-2xl p-5 space-y-2">
                  ...
                  <div className="flex justify-between py-1">
                    ...
                    <span className="text-sm font-medium text-slate-700">
                      {formData.rooms.reduce((acc, r) => acc + r.quantity, 0)} chambre{formData.rooms.reduce((acc, r) => acc + r.quantity, 0) > 1 ? 's' : ''}
                    </span>
                  </div>
