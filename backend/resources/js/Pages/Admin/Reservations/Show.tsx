          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <Section title="Détails du séjour" icon={Calendar}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
                <InfoRow icon={Calendar} label="Date d'arrivée" value={formatDate(reservation.date_arrivee)} />
                <InfoRow icon={Calendar} label="Date de départ" value={formatDate(reservation.date_depart)} />
                <InfoRow icon={Users} label="Nombre de personnes" value={`${reservation.nb_personnes} personne${reservation.nb_personnes !== 1 ? "s" : ""}`} />
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-100">
                        <th className="text-left py-2 text-xs text-slate-400 font-semibold uppercase tracking-wide">Type de chambre</th>
                        <th className="text-center py-2 text-xs text-slate-400 font-semibold uppercase tracking-wide">Qté</th>
                        <th className="text-right py-2 text-xs text-slate-400 font-semibold uppercase tracking-wide">Prix unitaire</th>
                        <th className="text-right py-2 text-xs text-slate-400 font-semibold uppercase tracking-wide">Sous-total</th>
                      </tr>
                    </thead>
                        ...
                        <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                          <td className="py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-medium text-slate-800">{item.type?.nom ?? `Type #${item.id_type}`}</span>
                            </div>
                          </td>
                    ...
                    <tfoot>
                      <tr className="border-t-2 border-slate-200">
                        <td colSpan={3} className="py-3 text-right text-sm font-bold text-slate-700">Total</td>
                        <td className="py-3 text-right text-base font-bold text-emerald-700">{formatPrice(reservation.prix_total)}</td>
                      </tr>
                    </tfoot>
                  </table>
                </div>
