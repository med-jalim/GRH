...
export default function ReservationsIndex({ reservations, filters, stats }: Props) {
  ...
  function applyStatutFilter(statut: string) {
    ...
    router.get(
      "/admin/reservations",
      { statut: statut === "all" ? "" : statut, id_hotel: filters.id_hotel ?? "" },
  ...
  function handleDelete(id: number, ref: string) {
    if (!confirm(`Supprimer la réservation ${ref} ? Cette action est irréversible.`)) return;
    setDeletingId(id);
  ...
  return (
    <AdminLayout>
      ...
      <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Réservations</h1>
          <p className="text-slate-500 mt-1 text-sm">
            {reservations.total} réservation{reservations.total !== 1 ? "s" : ""} au total
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: "Confirmées", value: stats.confirme, color: "text-emerald-700", bg: "bg-emerald-50" },
          { label: "Annulées", value: stats.annule, color: "text-red-700", bg: "bg-red-50" },
        ].map((s) => (
          <div
            key={s.label}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-[0_2px_8px_-4px_rgba(0,0,0,0.08)] overflow-hidden">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 px-8 text-center">
            ...
            <h3 className="text-lg font-semibold text-slate-700">Aucune réservation trouvée</h3>
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map((r, idx) => (
                  ...
                  >
                    ...
                    {/* Contact */}
                    <td className="px-5 py-4">
                      <p className="font-semibold text-slate-800">{r.nom_contact}</p>
                      {r.nom_agence && (
                        <p className="text-xs text-slate-400 mt-0.5">{r.nom_agence}</p>
                      )}
                    ...
                    <td className="px-5 py-4">
                      {r.hotel ? (
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="font-medium text-slate-700 text-xs">{r.hotel.name}</p>
                            <p className="text-xs text-slate-400">{r.hotel.ville}</p>
                          </div>
                        </div>
                      ) : (
                    ...
                    {/* Dates */}
                    <td className="px-5 py-4">
                      <p className="text-slate-700 font-medium text-xs">{formatDate(r.date_arrivee)}</p>
                      <p className="text-slate-400 text-xs">→ {formatDate(r.date_depart)}</p>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-1 text-slate-600">
                        ...
                        <span className="font-medium text-xs">{r.nb_personnes}</span>
                      </div>
                    </td>

                    {/* Prix */}
                    <td className="px-5 py-4">
                      <span className="font-semibold text-slate-800 text-sm">{formatPrice(r.prix_total)}</span>
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          ...
                          onClick={() => handleDelete(r.id, r.code_reference)}
        ...
        {reservations.last_page > 1 && (
          <div className="flex items-center justify-between px-5 py-4 border-t border-slate-100 bg-slate-50/50">
            <p className="text-xs text-slate-500">
              Affichage de <span className="font-semibold">{reservations.from}</span> à{" "}
              <span className="font-semibold">{reservations.to}</span> sur{" "}
              <span className="font-semibold">{reservations.total}</span> réservations
            </p>
